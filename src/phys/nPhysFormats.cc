...
: nPhysImageF<short>(std::string(ifilename), PHYS_FILE)
{
	...
	if ( std::string(ptr,3) != std::string("PCO")) {
	...
	for (int i=0; i<h; i++) {
		...
		//memcpy(Timg_matrix[i], readb, );
		set_Tvector(PHYS_X, i, 0, (short *)readb, w, PHYS_POS);
	}
...
: nPhysImageF<unsigned int>(ifilename, PHYS_FILE)
{
	...
	std::copy(std::istream_iterator<unsigned int>(ifile), std::istream_iterator<unsigned int>(), buf_itr());
...
void phys_dump_binary(physD *my_phys, std::ofstream &ofile) {
	...
	if (!ofile.fail() && my_phys != NULL) {
		...
		strm.next_in = (unsigned char *) my_phys->data_pointer();
...
std::vector <physD *> phys_open_tiff(std::string ifilename, bool separate_rgb) {
					...
					for (int k=0;k<samples;k++) {
						...
						for (tiff_uint32 j = 0; j < h; j++) {
							...
							for (tiff_uint32 i=0; i<w; i++) {
								...
								my_phys->set(i,j,my_phys->point(i,j,0)+val);
							}
						}
...
#ifdef HAVE_LIBTIFF
void phys_write_one_tiff(physD *my_phys, TIFF* tif) {
...
void phys_write_tiff(physD *my_phys, std::string ofilename) {
#ifdef HAVE_LIBTIFF
	augment_libtiff_with_custom_tags();
...
void phys_write_tiff(std::vector <physD *> vecPhys, std::string ofilename) {
#ifdef HAVE_LIBTIFF
	augment_libtiff_with_custom_tags();
	TIFF* tif = TIFFOpen(ofilename.c_str(), "w");
	if (tif) {
...
std::vector <physD *> phys_open_spe(std::string ifilename) {
	std::vector <physD *> vecReturn;
...
std::vector <physD *> phys_open_pcoraw(std::string ifilename) {
	std::vector <physD *> vecReturn;
...
std::vector <physD *> phys_open_inf(std::string ifilename) {
	...
	if (ifile) {
		...
		physD *linearized = new physD(w,h,0.0,ifilename);
		//linearized->setType(PHYS_FILE);
		linearized->setShortName("linearized");
		physD *original = new physD(w,h,0.0,ifilenameimg.c_str());
...
void phys_write_fits(physD *phys, const char * fname, float compression) {
#ifdef HAVE_LIBCFITSIO
	...
	// write actual data
	fits_write_img(fptr, TDOUBLE, 1, phys->getSurf(), (void *)phys->data_pointer(), &status);
	if (fits_check_error(status)) return;
...
std::vector <physD *> phys_open_fits(std::string ifilename) {
	...
	for (; !status; hdupos++)  {
		...
		physD *myphys=new physD(ifilename,PHYS_FILE);
		...
		if (anaxis==2) {
			...
			fits_read_pix(fptr, TDOUBLE, &fpixel[0], totalsize, NULL, (void *)myphys->data_pointer(), NULL, &status);
			if (fits_check_error(status)) return retVec;
...
std::vector <physD *> phys_resurrect_binary(std::string fname) {
...
int
phys_resurrect_binary(physD * my_phys, std::ifstream &ifile) {
	...
	std::vector<double> datavec(my_w*my_h);
	double *databuf = datavec.data();

	strm.next_in = &in[0];
	strm.avail_in = buffer_size;
	strm.next_out = (unsigned char *)databuf;
	...
	my_phys->swap_vector(my_w, my_h, datavec);
...
void phys_open_RAW(physD * my_phys, int kind, int skipbyte, bool endian){
...
std::vector <physD *> phys_open_HDF4(std::string fname) {
	std::vector <physD *> imagelist;
	DEBUG("HERE");
			...
			if (data.size()>0) {
				...
				if (status!=FAIL && rank>1) {
					...
					if (dim_sizes[rank-1]*dim_sizes[rank-2]>0) {
						...
						for (i=0;i<numMat;i++) {
							DEBUG(5,"	  mat " << i);
							physD *my_data = new physD(dim_sizes[rank-1],dim_sizes[rank-2],0.0,"hdf4");
...
void phys_write_HDF4(physD *phys, const char* fname) {
#if defined(HAVE_LIBMFHDF) || defined(HAVE_LIBMFHDFDLL)	
	if (phys) {
		intn istat=0;
