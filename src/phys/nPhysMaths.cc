...
inline void planeFit(physD *pi, double *coeffs)
{
	...
	for (size_t i=0; i<pi->getH(); i++) {
		for (size_t j=0; j<pi->getW(); j++) {
			...
			(y->data)[i*pi->getW()+j] = pi->point(i*pi->getW()+j) - pi->point(0);
...
phys_generate_morlet(morlet_data *md, physD &xx, physD &yy, physC &zz)
{
	...
	for (size_t i=0; i<zz.getW(); i++) {
		for (size_t j=0; j<zz.getH(); j++) {
			double x = xx.point(i,j);
			double y = yy.point(i,j);
			double rx = (cr*x - sr*y); double ry = (sr*x + cr*y);
			...
			zz.point(i,j) = mcomplex(mult*cos(kappa*rx), -mult*sin(kappa*rx));
...
void 
phys_add(physD &iimage, double val) { 
	if (val!=0.0) {
...
void 
phys_subtract(physD &iimage, double val) {
	if (val!=0.0) {
#pragma omp parallel for
		for (size_t ii=0; ii<iimage.getSurf(); ii++)
			iimage.set(ii, iimage.point(ii)-val);
...
void 
phys_multiply(physD &iimage, double val) {
	if (val!=1.0) {
#pragma omp parallel for
		for (size_t ii=0; ii<iimage.getSurf(); ii++)
			iimage.set(ii, iimage.point(ii)*val);
...
void 
phys_divide(physD &iimage, double val) {
	if (val!=1.0) {
#pragma omp parallel for
		for (size_t ii=0; ii<iimage.getSurf(); ii++)
			iimage.set(ii, iimage.point(ii)/val);
...
void
phys_divide(physC &iimage, double val) {
	if (val!=1.0) {
#pragma omp parallel for
		for (size_t ii=0; ii<iimage.getSurf(); ii++)
			iimage.set(ii, iimage.point(ii)/val);
...
void 
phys_point_add(physD &iimage, physD &iimage2) {
...
void 
phys_point_subtract(physD &iimage, physD &iimage2) {
	if (iimage.getSurf()==iimage2.getSurf()) {
#pragma omp parallel for
		for (size_t ii=0; ii<iimage.getSurf(); ii++)
			iimage.set(ii, iimage.point(ii)-iimage2.point(ii));
...
void 
phys_point_multiply(physD &iimage, physD &iimage2) {
	if (iimage.getSurf()==iimage2.getSurf()) {
#pragma omp parallel for
		for (size_t ii=0; ii<iimage.getSurf(); ii++)
			iimage.set(ii, iimage.point(ii)*iimage2.point(ii));
...
void 
phys_point_divide(physD &iimage, physD &iimage2) {
	if (iimage.getSurf()==iimage2.getSurf()) {
#pragma omp parallel for
		for (size_t ii=0; ii<iimage.getSurf(); ii++)
			iimage.set(ii, iimage.point(ii)/iimage2.point(ii));
...
double phys_sum_points(physD &iimage) {
	double retVal=0.0;
...
double phys_sum_square_points(physD &iimage) {
	double retVal=0.0;
#pragma omp parallel for reduction(+:retVal)
	for (size_t ii=0; ii<iimage.getSurf(); ii++) {
...
void phys_opposite(physD &iimage) {
#pragma omp parallel for
	for (size_t ii=0; ii<iimage.getSurf(); ii++) {
...
void phys_inverse(physD &iimage) {
#pragma omp parallel for
	for (size_t ii=0; ii<iimage.getSurf(); ii++) {
...
void phys_replace(physD &iimage, double oldval, double newval) {
#pragma omp parallel for
	for (size_t ii=0; ii<iimage.getSurf(); ii++) {
...
void phys_replace_NaN(physD &iimage, double newval) {
#pragma omp parallel for
	for (size_t ii=0; ii<iimage.getSurf(); ii++) {
...
void phys_cutoff(physD &iimage, double minval, double maxval) {
...
void phys_gauss_laplace(physD &image, double radius) {
...
void phys_gauss_sobel(physD &image, double radius) {
	phys_fast_gaussian_blur(image, radius);
	phys_sobel(image);
}

void phys_median_filter(physD& image, unsigned int N){
	physD my_copy=image;
...
std::pair<double, bidimvec<int> > phys_cross_correlate(physD* img1, physD* img2) {
	size_t dx=img1->getW();
	size_t dy=img1->getH();
	double maxValue=-1.0;
	...
	if (dx == img2->getW() && dy== img2->getH()) {
		...
		fftw_plan plan1R2C=fftw_plan_dft_r2c_2d(dy,dx, (double *)img1->data_pointer(), myData1C, FFTW_ESTIMATE);
		fftw_plan plan2R2C=fftw_plan_dft_r2c_2d(dy,dx, (double *)img2->data_pointer(), myData2C, FFTW_ESTIMATE);

		std::vector<double> r_datavec(dx*dy);
		double *r_dataptr = r_datavec.data();
		//fftw_plan planC2R=fftw_plan_dft_c2r_2d(dy,dx, myData1C, rPhys->Timg_buffer, FFTW_ESTIMATE);
		fftw_plan planC2R=fftw_plan_dft_c2r_2d(dy,dx, myData1C, r_dataptr, FFTW_ESTIMATE);
		...
		rPhys->swap_vector(dx, dy, r_datavec);

		for (size_t i=0;i<dx;i++) {
			for (size_t j=0;j<dy;j++) {
				if (rPhys->point(i,j) > rPhys->point(maxP.x(),maxP.y(), 0)) {
					maxP=vec2f(i,j);
				}
			}
		}
		maxValue=rPhys->point(maxP.x(),maxP.y(), 0);
...
bidimvec<size_t> phys_max_p(physD &image) {
...
std::map<std::string, physD > to_powersp(physC &iphys, bool doLog) {
	physD psp;
	std::map<std::string, physD > omap;
...
physC from_real_imaginary (physD& real, physD&imag) {
	physC ret;
	if (real.getSurf() == imag.getSurf()) {
...
physC from_real (physD&real, double val){
	physC ret;
	ret.resize(real.getW(),real.getH());
#pragma omp parallel for
	for (size_t ii=0; ii<real.getSurf(); ii++) {
...
// contour functions
//
void contour_trace(physD &iimage, std::list<vec2> &contour, float level, bool blur, float blur_radius)
{
	...
	for (size_t ii=0; ii<cmap.getSurf(); ii++) {
		size_t xx = ii%cmap.getW();
		size_t yy = ii/cmap.getW();
	...
	for (size_t ii=0; ii<cmap.getW(); ii++) {
		cmap.set(ii, 0, cmap.point(ii, 0, 0) &~ 12);
		cmap.set(ii, 0, cmap.point(ii, cmap.getH()-1, 0) &~ 3);
	}
	for (size_t ii=0; ii<cmap.getH(); ii++) {
		cmap.set(0, ii, cmap.point(0, ii, 0) &~ 9);
	...
	int ls_x = orig.x();
	while (ls_x > -1 && cmap.point(ls_x, orig.y(), 0) == 15)
		ls_x--;
	stats[cmap.point(ls_x, orig.y(), 0)]--;
	...
	*itr = vec2(ls_x, orig.y());
	...
	while (itr != contour.end()) {
		...
		if (val==5 || val==10) {
			short central = ((.25*wimage.point(xx,yy, 0) + wimage.point(xx+1,yy+1, 0) + wimage.point(xx+1,yy, 0) + wimage.point(xx,yy+1, 0)) > level) ? 1 : -1;
...
std::list<double> contour_integrate(physD &iimage, std::list<vec2> &contour, bool integrate_boundary)
{
	...
	for (int xx=starting_point.x(); intg_image.point(xx, starting_point.y(), 0) != check_val; xx++) {
	...
	if (integrate_boundary) {
		...
		double bps = 0;
		for (std::list<vec2>::iterator itr = contour.begin(); itr != contour.end(); ++itr) {
			bps+= iimage.point((*itr).x(), (*itr).y(), 0);
			points_count++;
		}
...
physC& ft2(physC& iimg, enum phys_fft) {
	DEBUG("---------------------------------- WARNING ---------------------------------------");
	DEBUG("------------------------this is a placeholder function!!--------------------------");
}

physC& ft2(physD& iimg, enum phys_fft) {
	DEBUG("---------------------------------- WARNING ---------------------------------------");
	DEBUG("------------------------this is a placeholder function!!--------------------------");
}
