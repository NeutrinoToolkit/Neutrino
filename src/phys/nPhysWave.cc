...
void phys_wavelet_field_2D_morlet(wavelet_params &params)
{
	...
	if (params.data && (params.n_angles > 0) && (params.n_lambdas > 0) && (params.data->getSurf() != 0)) {
		...
		physD *qmap, *wphase, *lambda, *angle, *intensity;
		qmap = new physD(dx, dy, 0.0, "quality");

		wphase = new physD("phase/2pi");
		wphase->resize(dx, dy);
		lambda = new physD("lambda");
		lambda->resize(dx, dy);
		angle = new physD("angle");
		angle->resize(dx, dy);
		intensity = new physD("intensity");
		...
		nPhysImageF<mcomplex> Fmain_window=ft2(*params.data, PHYS_FORWARD);
...
#pragma omp parallel for
				for (size_t k=0;k<zz_morlet.getSurf();k++) {
					...
					zz_morlet.point(k)=Fmain_window.point(k)*gauss;
				}
				//nPhysImageF<mcomplex> zz_convolve = zz_morlet.ft2(PHYS_BACKWARD);
				nPhysImageF<mcomplex> zz_convolve = ft2(zz_morlet, PHYS_BACKWARD);

				// decision
#pragma omp parallel for
				for (size_t k=0; k<params.data->getSurf(); k++) {
					double qmap_local=zz_convolve.point(k).mcabs();
					if ( qmap_local > qmap->point(k)) {
						qmap->set(k, qmap_local);
						wphase->set(k, zz_convolve.point(k).arg());
						lambda->set(k, lambdas[i]);
						angle->set(k, angles[j]);
					}
				}
...
#pragma omp parallel for
			for (size_t k=0; k<params.data->getSurf(); k++) {
				qmap->set(k, sqrt(qmap->point(k))/(dx*dy));
				intensity->set(k, params.data->point(k) - 2.0*qmap->point(k)*cos(wphase->point(k)));
				wphase->set(k, wphase->point(k)/(2*M_PI));
			}
...
#pragma omp parallel for
				for (size_t k=0; k<params.data->getSurf(); k++) {
					if(!std::isfinite(params.data->point(k))){
...
void phys_wavelet_field_2D_morlet_opencl(wavelet_params &params) {
	...
	if (params.opencl_unit>0) {
		...
		*params.iter_ptr=0;
		...
		bidimvec<int> offset=(newSize-params.data->getSize())/2;
...
#pragma omp parallel for collapse(2)
		...
		for(cl_uint j=0; j<dy; j++) {
			for(cl_uint i=0; i<dx; i++) {
				inReal[j*dx+i] = padded.point(i,j, 0);
			}
		}
...
// unwrap methods
	physD *
phys_phase_unwrap(physD &wphase, physD &quality, enum unwrap_strategy strategy)
{
	...
	physD *uphase;
	uphase = new physD (wphase.getW(), wphase.getH(), 0., "unwrap");
...
#pragma omp parallel for
	...
	void
phys_subtract_carrier(physD &iphys, double kx, double ky)
{

#pragma omp parallel for collapse(2)
	for (size_t ii=0; ii<iphys.getW(); ii++) {
		for (size_t jj=0; jj<iphys.getH(); jj++) {
			iphys.set(ii, jj, iphys.point(ii, jj)-ii*kx - jj*ky);
		}
	}
...
//! this function returns the carrier bidimvec<double(angle[deg],interfringe[px])>
	bidimvec<double>
phys_guess_carrier(physD &phys, double weight)
{
	size_t dx=phys.getW();
	size_t dy=phys.getH();

	fftw_complex *myData=fftw_alloc_complex(dy*(dx/2+1));
	fftw_plan plan=fftw_plan_dft_r2c_2d(dy,dx, (double *)phys.data_pointer(), myData, FFTW_ESTIMATE);
	fftw_execute(plan);
...
// --------------------------------------------------------------------- integral inversions --
	...
	void
phys_apply_inversion_gas(physD &invimage, double probe_wl, double res, double molar_refr)
{
	...
	double mult = GSL_CONST_NUM_AVOGADRO / (3*molar_refr);
	...
	void
phys_apply_inversion_plasma(physD &invimage, double probe_wl, double res)
{
	double kappa = 2*M_PI/probe_wl;
	double mult = GSL_CONST_MKSA_MASS_ELECTRON*GSL_CONST_MKSA_VACUUM_PERMITTIVITY*GSL_CONST_MKSA_SPEED_OF_LIGHT*GSL_CONST_MKSA_SPEED_OF_LIGHT/(GSL_CONST_MKSA_ELECTRON_CHARGE*GSL_CONST_MKSA_ELECTRON_CHARGE);
	...
	void
phys_apply_inversion_protons(physD &invimage, double energy, double res, double distance, double magnification)
{
...
void phys_invert_abel(abel_params &params)
{
	...
	params.oimage = new physD (params.iimage->getW(), params.iimage->getH(),std::numeric_limits<double>::quiet_NaN(),"Inverted");
