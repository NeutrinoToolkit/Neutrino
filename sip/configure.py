...
# This must be kept in sync with neutrino.pro

neutrino_version = "1.0.0"
...
class TargetConfiguration:
    ...
    def get_qt_configuration(self, opts):
        ...
        self.neu_default_inc_dir = qt_config.QT_INSTALL_HEADERS
        self.neu_inc_dir = None
        self.neu_default_lib_dir = qt_config.QT_INSTALL_LIBS
        self.neu_lib_dir = None
    ...
    def override_defaults(self, opts):
        ...
        if opts.neu_inc_dir is not None:
            self.neu_inc_dir = opts.neu_inc_dir

        if opts.neu_lib_dir is not None:
            self.neu_lib_dir = opts.neu_lib_dir
        ...
        if opts.qneu_sip_dir is not None:
            self.qneu_sip_dir = opts.qneu_sip_dir
        else:
            self.qneu_sip_dir = self.pyqt_sip_dir
        ...
        if opts.is_dll is not None:
            self.neuntilla_is_dll = opts.is_dll
...
def create_optparser(target_config):
    ...
    p = optparse.OptionParser(usage="python %prog [options]",
            version="2.8.2-snapshot-d68e28068b67")

    p.add_option("--spec", dest='qmakespec', default=None, action='store',
            metavar="SPEC",
            help="pass -spec SPEC to qmake [default: %s]" % "don't pass -spec" 
            if target_config.qmake_spec == '' else target_config.qmake_spec)
    p.add_option("--apidir", "-a", dest='apidir', type='string', default=None,
            action='callback', callback=store_abspath, metavar="DIR", 
            help="the PyNeutrino API file will be installed in DIR [default: "
                    "QT_INSTALL_DATA/neu]")
    ...
    p.add_option("--destdir", "-d", dest='destdir', type='string',
            default=None, action='callback', callback=store_abspath,
            metavar="DIR",
            help="install the PyNeutrino module in DIR [default: "
    ...
    p.add_option("--neu-incdir", "-n", dest='neu_inc_dir', type='string',
            default=None, action='callback', callback=store_abspath_dir,
            metavar="DIR",
            help="the directory containing the PyNeutrino PyNeu header file "
                    "directory is DIR [default: QT_INSTALL_HEADERS]")
    p.add_option("--neu-libdir", "-o", dest='neu_lib_dir', type='string',
            default=None, action='callback', callback=store_abspath_dir,
            metavar="DIR",
            help="the directory containing the PyNeutrino library is DIR "
    ...
    p.add_option("--neu-sipdir", "-v", dest='qneu_sip_dir', type='string',
            default=None, action='callback', callback=store_abspath_dir,
            metavar="DIR",
            help="the PyNeutrino .sip files will be installed in DIR "
                    "[default: %s]" % target_config.qneu_sip_dir)
    ...
    p.add_option("--static", "-k", dest='static', default=False,
            action='store_true',
            help="build the PyNeutrino module as a static library")
    ...
    p.add_option("--trace", "-r", dest='tracing', default=False,
            action='store_true',
            help="build the PyNeutrino module with tracing enabled")
    p.add_option("--no-dll", "-s", dest='is_dll', default=None,
            action='store_false',
            help="PyNeutrino is a static library and not a Windows DLL")
    p.add_option("--debug", "-u", default=False, action='store_true',
            help="build the PyNeutrino module with debugging symbols")
...
def inform_user(target_config):
    ...
    inform("The PyNeutrino module will be installed in %s." % target_config.module_dir)

    if target_config.prot_is_public:
        inform("The PyNeutrino module is being built with 'protected' "
                "redefined as 'public'.")

    inform("The PyNeutrino .sip files will be installed in %s." %
            target_config.qneu_sip_dir)

    inform("The PyNeutrino API file will be installed in %s." %
...
def generate_code(target_config, opts):
    """ Generate the code for the PyNeutrino module.  target_config is the
    target configuration.  opts are the command line options.
    """

    inform("Generating the C++ source for the PyNeu module...")
...
#         argv.append('sip/neumod5.sip')
#     else:
#         argv.append('sip/neumod4.sip')
...
def generate_pro(target_config, opts):
    """ Generate the .pro file for the PyNeutrino module.  target_config is the
    target configuration.  opts are the command line options.
    """
    ...
    inform("Generating the .pro file for the PyNeu module...")

    pro = open('PyNeutrino.pro', 'w')

    pro.write('TEMPLATE = lib\n')
    pro.write('CONFIG += %s\n' % ('debug' if opts.debug else 'release'))
    pro.write('CONFIG += %s\n' % ('staticlib' if opts.static else 'plugin'))
    pro.write('CONFIG += Neutrino\n')
    ...
    if not opts.static:
        ...
        pro.write('''
win32 {
    PY_MODULE = PyNeutrino.pyd
    target.files = PyNeu.pyd
    LIBS += -L%s
} else {
    PY_MODULE = PyNeutrino.so
    target.files = PyNeutrino.so
}
...
api.files = PyNeutrino.api
INSTALLS += api
''' % target_config.api_dir)

    pro.write('''
sip.path = %s/PyNeutrino
sip.files =''' % target_config.qneu_sip_dir)
    for s in glob.glob('*.sip'):
        pro.write(' \\\n    %s' % s)
    pro.write('''
INSTALLS += sip
''')
    ...
    if target_config.py_platform.startswith('linux'):
        ...
        if not opts.static:
            if target_config.py_version >= 0x030000:
                entry_point = 'PyInit_PyNeutrino'
            else:
                entry_point = 'initPyNeutrino'

            exp = open('PyNeutrino.exp', 'wt')
            exp.write('{ global: %s; local: *; };' % entry_point)
            exp.close()

            pro.write('QMAKE_LFLAGS += -Wl,--version-script=PyNeutrino.exp\n')

    if target_config.prot_is_public:
        pro.write('DEFINES += SIP_PROTECTED_IS_PUBLIC protected=public\n')

    if target_config.neuntilla_is_dll:
        pro.write('DEFINES += neuNTILLA_DLL\n')

    if target_config.neu_inc_dir is not None:
        pro.write('INCLUDEPATH += %s\n' % target_config.neu_inc_dir)

    pro.write('INCLUDEPATH += %s\n' % target_config.py_inc_dir)
    if target_config.py_inc_dir != target_config.sip_inc_dir:
        pro.write('INCLUDEPATH += %s\n' % target_config.sip_inc_dir)

    lib_dir = target_config.neu_lib_dir
    if lib_dir is None:
        lib_dir = target_config.neu_default_lib_dir
    else:
        pro.write('LIBS += -L%s -lNeutrino\n' % lib_dir)
...
''' % (neutrino_version, lib_dir, neutrino_version))
    ...
    pro.write('\n')
    pro.write('TARGET = PyNeutrino\n')
    pro.write('HEADERS = sipAPIPyNeutrino.h\n')
...
def check_sip(target_config):
    ...
    if 'snapshot' not in version_str:
        ...
        if version < min_version:
            error(
                    "This version of PyNeutrino requires sip %s or later." %
                            SIP_MIN_VERSION)
