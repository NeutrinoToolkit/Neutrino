...
void phys_wavelet_field_2D_morlet(wavelet_params &params)
{
    ...
    if (params.data && (params.n_angles > 0) && (params.n_lambdas > 0) && (params.data->getSurf() != 0)) {
        ...
        *params.iter_ptr=0;
        ...
        fftw_complex *temp = fftw_alloc_complex(surf);      // used for holding both the image for the forward fftw and the convoluted with the gaussian
        fftw_complex *Ft = fftw_alloc_complex(surf);        // holds the fft of the image
        fftw_complex *morlet = fftw_alloc_complex(surf);    // this is the real convolution

        fftw_plan plan_fwd = fftw_plan_dft_2d(dy, dx, temp, Ft, FFTW_FORWARD, FFTW_ESTIMATE);
        fftw_plan plan_bwd = fftw_plan_dft_2d(dy, dx, temp, morlet, FFTW_BACKWARD, FFTW_ESTIMATE);

#pragma omp parallel for
        for (size_t i = 0; i < surf; i++) {
            temp[i][0] = params.data->Timg_buffer[i];
            temp[i][1] = 0;
        }
...
#pragma omp parallel for collapse(2)
                for (unsigned int y=0;y<dy;y++) {
                    for (unsigned int x=0;x<dx;x++) {
                        ...
                        unsigned int xx=(x+(dx+1)/2)%dx-(dx+1)/2;
                        unsigned int yy=(y+(dy+1)/2)%dy-(dy+1)/2;
                        ...
                        temp[k][0]=Ft[k][0]*gauss;
                        temp[k][1]=Ft[k][1]*gauss;
...
#pragma omp parallel for
                for (size_t k=0; k<params.data->getSurf(); k++) {
                    double qmap_local=pow(morlet[k][0],2)+pow(morlet[k][1],2);
                    if ( qmap_local > qmap->Timg_buffer[k]) {
                        qmap->Timg_buffer[k] = qmap_local;
                        wphase->Timg_buffer[k] = atan2(morlet[k][1], morlet[k][0]);
