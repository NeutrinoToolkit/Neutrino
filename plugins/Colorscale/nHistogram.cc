...
void nHistogram::drawPicture (QPainter &p) {
    ...
    if (parentPan->nparent->my_view->nPalettes[parentPan->nparent->my_view->colorTable].size()) {
        for (int i=0; i<256; i++) {
            QColor colore=QColor((int)parentPan->nparent->my_view->nPalettes[parentPan->nparent->my_view->colorTable][3*i+0],(int)parentPan->nparent->my_view->nPalettes[parentPan->nparent->my_view->colorTable][3*i+1],(int)parentPan->nparent->my_view->nPalettes[parentPan->nparent->my_view->colorTable][3*i+2]);
            p.setBrush(colore);
