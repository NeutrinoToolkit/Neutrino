...
Affine::Affine(neutrino *nparent) : nGenericPan(nparent)
{
	my_w.setupUi(this);
    l1 =  new nLine(this,1);
...
void Affine::bufferChanged(nPhysD* buf) {
...
vec2f Affine::affine(vec2f in, std::vector<double> vec){
	return vec2f(in.x()*vec[0]+in.y()*vec[1]+vec[2],in.x()*vec[3]+in.y()*vec[4]+vec[5]);
}

std::vector<double> Affine::getAffine(QPolygonF poly1, QPolygonF poly2) {
