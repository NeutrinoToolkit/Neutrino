 ...
 *
 */
#include "IntegralInversion.h"
#include "neutrino.h"

IntegralInversion::IntegralInversion(neutrino *nparent)
: nGenericPan(nparent),
...
void IntegralInversion::physDel(nPhysD* buf) {
    if (buf==invertedPhys) {
        invertedPhys=NULL;
    }
}
...
void IntegralInversion::refphase_checkbChanged(int val) {
	my_w.refphase_cb->setEnabled(val==2);
}

void
IntegralInversion::sceneChanged()
{
...
QVariant IntegralInversion::doInversion() {
	saveDefaults();
	QVariant retVar;
	...
	if (image) {
        ...
        if (inv_image) {
            ...
            DEBUG("[IntegralInversion] Error: inversion returned NULL");
		}
