 ...
 *
 */
#include "BoxLineout.h"
    
BoxLineout::BoxLineout(neutrino *nparent) : nGenericPan(nparent)
{
	my_w.setupUi(this);
...
void BoxLineout::sceneChanged() {
	if (sender()==box) updatePlot();
}

void BoxLineout::mouseAtWorld(QPointF p) {
	if (currentBuffer) {
        my_w.plot->setMousePosition(p.x(),p.y());
	}
}

void BoxLineout::updatePlot() {
	if (currentBuffer) {
