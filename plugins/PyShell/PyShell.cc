...
bool PyShellPlug::instantiate(neutrino *neu) {
    qDebug() << "here" << neu;
    nparent=neu;
...
PyShell::PyShell(neutrino *nparent) : nGenericPan(nparent)
{
	my_w.setupUi(this);
...
void PyShell::runScript(QString cmd) {
    saveDefaults();
    DEBUG("\n" << cmd.toStdString());
...
void PyShell::changeScriptsFolder() {
